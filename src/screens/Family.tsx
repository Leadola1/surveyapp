...
export default function Farmer() {
  ...
  const savedFormValues = useSelector(
    (state: RootState) => state.family.formValues,
  );
  ...
  const renderFormFields = () => {
    return (
      <>
        {formData.farmer_family_registration.map(field => {
          if (field.visible === 1) {
            return (
              <FormField
  ...
  const handleSubmit = () => {
    const mandatoryFields = formData.farmer_family_registration.filter(
    ...
    if (valuesProvided) {
      const nonMandatoryFields = formData.farmer_family_registration.filter(
      ...
      const orderedKeys = [
        ...
        ...Object.keys(formValues)
          .sort()
          .filter(key => key !== 'Name'), // Exclude 'Name' to avoid duplication
      ];
